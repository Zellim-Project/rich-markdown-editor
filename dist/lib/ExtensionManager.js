...
class ExtensionManager {
    ...
    get plugins() {
        return (0, uniqBy_1.default)(this.extensions.reduce((allPlugins, extension) => {
    ...
    keymaps({ schema }) {
        ...
        console.log(keymaps.filter(key => Object.keys(key).length > 0), keymaps.filter(key => Object.keys(key).length > 0).map(prosemirror_keymap_1.keymap));
        return keymaps.filter(key => Object.keys(key).length > 0).map(prosemirror_keymap_1.keymap);
    }
    inputRules({ schema }) {
