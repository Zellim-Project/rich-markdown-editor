...
import embedATaskCommand from "../commands/embedATask";
import linkDocumentCommand from "../commands/linkDocument";
import embedAProjectCommand from "../commands/embedAProject";
...
class CommandMenu extends React.Component {
    constructor() {
        ...
        this.embedTaskRef = React.createRef();
        this.embedProjectRef = React.createRef();
        this.linkDocumentRef = React.createRef();
        this.state = {
            left: -1000,
            top: 0,
            bottom: undefined,
        ...
        this.handleKeyDown = (event) => {
            ...
            if (event.key === "ArrowDown" ||
                ...
                if (this.filtered.length) {
                    ...
                    this.setState({
                        selectedIndex: Math.min(next?.name === "separator" ? nextIndex + 1 : nextIndex, total),
                    });
                }
                else {
                    this.close();
        ...
        this.handleFilePicked = (event) => {
            ...
            const parent = findParentNode((node) => !!node)(state.selection);
            if (parent) {
                dispatch(state.tr.insertText("\f", parent.pos, parent.pos + parent.node.textContent.length + 1));
        ...
        this.handleEmbedAProject = (event) => {
    ...
    get filtered() {
        const { embeds = [], search = "", uploadImage, uploadFile, embedATask, embedAProject, linkDocument, commands, filterable = true, } = this.props;
        ...
        const filtered = items.filter((item) => {
            ...
            if (!uploadImage && item.name === "image")
                return false;
            if (!uploadFile && item.name === "container_file")
                return false;
            if (!embedATask && item.name === "container_task")
                return false;
            if (!embedAProject && item.name === "container_project")
                return false;
            if (!linkDocument && item.name === "container_link_doc")
                return false;
            if (!search)
    ...
    render() {
        const { dictionary, isActive, uploadImage, uploadFile, embedATask, embedAProject, linkDocument, } = this.props;
        ...
        return (React.createElement(Portal, null,
            React.createElement(Wrapper, Object.assign({ id: this.props.id || "block-menu-container", active: isActive, ref: this.menuRef }, positioning),
                ...
                uploadFile && (React.createElement(VisuallyHidden, null,
                    React.createElement("input", { type: "file", ref: this.fileInputRef, onChange: this.handleFilePicked, accept: "*" }))),
                embedATask && (React.createElement(VisuallyHidden, null,
                    React.createElement("div", { ref: this.embedTaskRef, onClick: this.handleEmbedATask }))),
                embedAProject && (React.createElement(VisuallyHidden, null,
                    React.createElement("div", { ref: this.embedProjectRef, onClick: this.handleEmbedAProject }))),
                linkDocument && (React.createElement(VisuallyHidden, null,
                    React.createElement("div", { ref: this.linkDocumentRef, onClick: this.handleLinkDocument }))))));
    }
}
