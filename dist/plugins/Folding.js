        ...
        return [
            new Plugin({
                ...
                appendTransaction: (transactions, oldState, newState) => {
                    ...
                    for (const block of blocks) {
                        if (block.node.type.name === "heading") {
                            const persistKey = headingToPersistenceKey(block.node, this.editor.props.id);
                ...
                props: {
                    decorations: state => {
                        const { doc } = state;
                        const decorations = findCollapsedNodes(doc).map(block => Decoration.node(block.pos, block.pos + block.node.nodeSize, {
